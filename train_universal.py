...
方案1: U-Net从头训练 (--model_type unet)
方案2: Libcom预训练PCTNet (--model_type libcom)
方案3: ResNet-UNet with ImageNet预训练 (--model_type resnet)
"""
import os
import argparse
...
from mulit_models import get_model
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Validation")):
            ...
            if batch_idx == 0 and writer is not None:
                ...
                real_vis = (real[:4] + 1) / 2
                pred_vis = (pred[:4] + 1) / 2
                mask_vis = mask[:4].repeat(1, 3, 1, 1)
...
def main(args):
    ...
    exp_name = f"{args.exp_name}_{args.model_type}_{timestamp}"
    ...
    if args.model_type == 'unet':
        ...
        model_kwargs['pretrained'] = True  # 使用ImageNet预训练
    elif args.model_type == 'libcom':
        model_kwargs['use_simplified'] = args.use_simplified_libcom
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Universal training script for crack harmonization')

    # 模型选择
    parser.add_argument('--model_type', type=str, default='resnet',
                        choices=['unet', 'libcom', 'resnet'],
                        help='Model type: unet (方案1), libcom (方案2), resnet (方案3)')

    # 数据路径
    parser.add_argument('--train_image_dir', type=str, default='data_kengcao/train/images',
                        help='Training image directory')
    parser.add_argument('--train_mask_dir', type=str, default='data_kengcao/train/masks',
                        help='Training mask directory')
    parser.add_argument('--val_image_dir', type=str, default='data_kengcao/val/images',
                        help='Validation image directory')
    parser.add_argument('--val_mask_dir', type=str, default='data_kengcao/val/masks',
                        help='Validation mask directory')

    # 模型配置
    parser.add_argument('--pretrained_path', type=str, default='/home/enine/anaconda3/envs/DIH/lib/python3.10/site-packages/libcom/image_harmonization/pretrained_models/PCTNet.pt',
                        help='Path to pretrained model weights (仅用于unet)')
    parser.add_argument('--freeze_encoder', action='store_true',
                        help='Freeze encoder layers for fine-tuning')
    parser.add_argument('--use_simplified_libcom', action='store_true',
                        help='Use simplified libcom wrapper (仅用于libcom)')
    ...
    parser.add_argument('--exp_name', type=str, default='crack_harmonization',
